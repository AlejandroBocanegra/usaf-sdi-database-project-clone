...
const read = (request, response) => {
  ...
  switch (type) {
    ...
    case "purchaseOrder":
      pool.query("SELECT * FROM purchase_orders ORDER BY id ASC", (error, results) => {
        if (error) {
          throw error;
        }
        response.status(200).send(JSON.stringify(results.rows));
    ...
    pool.query(
      ...
      (error, result) => {
        ...
        response
          .status(201)
          .send(
            JSON.stringify(`User ${newUser.firstName} ${newUser.lastName} was successfully added.`)
          );
      }
    );
  }
    ...
    pool.query(
      ...
      (error, result) => {
        ...
        response
          .status(201)
          .send(
            JSON.stringify(`Manufacturer ${newManufacturer.company_name} was successfully added.`)
          );
      }
    );
  }
